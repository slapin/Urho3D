...
void Vehicle::PostUpdate(float timeStep)
{
    ...
    for (int i = 0; i < vehicle->GetNumWheels(); i++)
    {
        ...
        if (vehicle->WheelIsGrounded(i) && (
                    vehicle->GetWheelSkidInfoCumulative(i) < 0.9f ||
                    vehicle->GetBrake(i) > 2.0f ||
                    planeAccel > 15.0f))
        {
            ...
            URHO3D_LOGDEBUG("GetWheelSkidInfoCumulative() = " + String(vehicle->GetWheelSkidInfoCumulative(i)) + " " + String(vehicle->GetMaxSideSlipSpeed()));
